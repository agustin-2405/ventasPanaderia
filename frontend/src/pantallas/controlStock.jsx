...
export default function ControlStock() {
  ...
  const obtenerProductos = async () => {
    try {
      ...
      error("No se pudo cargar el inventario", err.message);
    } finally {
      setCargando(false);
    }
  };
  ...
  const modificarStock = async (productoId, cantidad, operacion) => {
    ...
    try {
      const respuesta = await fetch(url, {
        ...
        body: JSON.stringify({ productoId, cantidad: parseFloat(cantidad) }),
      });
      ...
      if (respuesta.ok) {
        ...
        setProductos(
          (prevProductos) =>
            prevProductos.map((p) =>
              p.id === productoId ? { ...p, stock: datos.stock } : p,
            ),
          exito(
            "Stock actualizado",
            `El stock de ${datos.nombre} ahora es ${datos.stock} unidades.`,
          ),
        );
      } else {
        error("No se pudo actualizar el stock", datos.error);
      }
    } catch (err) {
      error(
        "Error de conexión",
  ...
  if (cargando)
    return (
      <p style={estilos.centrado}>Cargando inventario de la panadería...</p>
    );
  if (mensajeError)
    return (
      <p style={{ ...estilos.centrado, color: "red" }}>Error: {mensajeError}</p>
    );
  ...
  return (
    <div style={estilos.contenedor}>
      ...
      <p style={estilos.subtitulo}>
        Aumentá o disminuí las existencias disponibles de la mercadería.
      </p>

      {productos.length === 0 ? (
        <p style={estilos.vacio}>
          No hay productos cargados. Usá la pantalla de Alta de Productos
          primero.
        </p>
      ) : (
