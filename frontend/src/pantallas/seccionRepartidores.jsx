...
export default function SeccionRepartidores() {
  ...
  const obtenerRepartidores = async () => {
    try {
      ...
      if (!respuesta.ok) throw new Error('No se pudo cargar la lista de repartidores.');
  ...
  const manejarEnvio = async (e) => {
    ...
    try {
      ...
      if (respuesta.ok) {
        setMensaje({ texto: `¡Repartidor ${datos.nombre} registrado con éxito!`, tipo: 'exito' });
        setNombre('');
        setTelefono('');
        setVehiculo('');
        ...
        setMensaje({ texto: `Error: ${datos.error}`, tipo: 'error' });
      }
    } catch (err) {
  ...
  return (
    <div style={estilos.contenedorPrincipal}>
      ...
      <div style={estilos.columnaForm}>
        ...
        <p style={estilos.subtitulo}>Da de alta al personal encargado de los repartos diarios.</p>
        ...
        <form onSubmit={manejarEnvio} style={estilos.formulario}>
          ...
          <button type="submit" style={estilos.boton}>⚡ Registrar Repartidor</button>
        </form>
      </div>
...
const estilos = {
  contenedorPrincipal: { display: 'flex', gap: '40px', maxWidth: '1000px', margin: '40px auto', padding: '0 20px', fontFamily: 'Arial, sans-serif' },
  columnaForm: { flex: '1', padding: '20px', border: '1px solid #ddd', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', height: 'fit-content' },
  columnaLista: { flex: '1', padding: '20px' },
  titulo: { margin: '0 0 10px 0', color: '#333' },
  tituloSeccion: { margin: '0 0 20px 0', color: '#2c3e50', borderBottom: '2px solid #34495e', paddingBottom: '10px' },
  ...
  input: { padding: '10px', fontSize: '15px', borderRadius: '4px', border: '1px solid #ccc' },
  boton: { padding: '12px', fontSize: '16px', backgroundColor: '#34495e', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', marginTop: '10px' },
  ...
  tarjeta: { display: 'flex', alignItems: 'center', gap: '15px', padding: '12px', borderRadius: '6px', backgroundColor: '#f8f9fa', border: '1px solid #e9ecef' },
  avatar: { fontSize: '24px', backgroundColor: '#e9ecef', padding: '8px', borderRadius: '50%' },
